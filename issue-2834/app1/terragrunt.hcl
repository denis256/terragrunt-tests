terraform {
  ...
  extra_arguments "conditional_vars" {
    ...
    required_var_files = concat(
      fileexists("${get_terragrunt_dir()}/${local.cluster_region}.tfvars") ? ["${get_terragrunt_dir()}/${local.cluster_region}.tfvars"] : [],
      fileexists("${get_terragrunt_dir()}/${local.cluster_region}2.tfvars") ? ["${get_terragrunt_dir()}/${local.cluster_region}2.tfvars"] : [],
      [
